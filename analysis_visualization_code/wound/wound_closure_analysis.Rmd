...
**Research Question**: Do damselfish (*Dascyllus flavicaudus*)
accelerate coral wound healing rates in *Pocillopora* spp.?

**Experimental Design**:

-   **Factors**: Fish presence (Present/Absent) × Wound size
    (None/Small/Large)
...
One fragment (coral #95; No Fish, Small wound, tank 2) exhibited net
tissue loss over the experiment: its wound expanded from 1.02 cm² to
2.09 cm² (`percent_healed` = −105%), yielding a negative "healing rate."
This is a qualitatively distinct outcome (wound expansion / partial
necrosis) rather than slow healing, so it is excluded from the
wound-healing-rate analysis and reported explicitly in the Methods
(n = 47 fragments retained for the healing-rate models). The exclusion
is disclosed rather than silently filtered, per Referee 1's emphasis on
statistical transparency.

```{r clean-percent-data}
# Process continuous healing rate data.
# Exclude coral #95: wound EXPANDED (1.02 -> 2.09 cm2; percent_healed = -105%).
# This is net tissue loss / necrosis, a different outcome than slow
# healing, not a data-entry error. Exclusion is disclosed in the
# manuscript Methods (n = 47 retained). See Section 4 for the
# interaction LRT and 6b for the censoring sensitivity context.
percent_closure <- percent_closure %>%
  filter(percent_healed >= 0) %>%  # excludes coral #95 (net tissue loss; see note above)
  prep_treatments() %>%
...
We fit linear mixed-effects models to test how fish presence and wound
size affect healing rate (continuous response variable in cm²/day).

**Model structure**: - **Fixed effects**: Fish presence, Wound size, and
their interaction - **Random effect**: Tank (accounts for shared tank
environment) - **Response**: Healing rate (cm²/day)

## Fit Candidate Models

```{r fit-healing-models}
# All models fit by maximum likelihood (REML = FALSE) so that likelihood
# ratio tests comparing nested fixed-effect structures are valid
# (per reviewer 1 request, RSBL-2026-0177 revision).
...
cat("✓ Models fitted successfully (ML / REML = FALSE)\n")
```
...
# Drop unused factor levels for proper positioning
percent_closure_plot <- percent_closure %>%
  mutate(wound = droplevels(wound))

# Calculate summary statistics for plotting
healing_summary <- percent_closure_plot %>%
  group_by(wound, fish) %>%
  summarize(
    mean_val = mean(healing_rate),
    sd_val = sd(healing_rate),
    n = n(),
    se = sd_val / sqrt(n),
...
gap_size <- 0.006  # Adjust based on y-axis scale

# Create publication-quality figure
# Grayscale compatible: No Fish = white fill, Fish = dark fill
healing_plot <- percent_closure_plot %>%
  ggplot(aes(x = wound, y = healing_rate, color = fish, fill = fish, shape = fish)) +
  ...
  labs(
    y = expression("Healing Rate (cm"^2*"/day)"),
    x = "Wound Size"
  ) +
  guides(
...
# 6b. Left-Censoring Sensitivity Check (R1.2)

## Rationale

Reviewer 1 asked whether any wounds reached complete closure (0 cm²)
before day 21. When that happens, the healing rate calculated as
`(initial − final) / 21` is biased downward, because the wound stopped
shrinking partway through and we are still dividing by 21 days. We
document the pattern here and discuss its direction.

## Closure cross-tab

```{r closure-crosstab}
closure_xtab <- percent_closure %>%
  filter(wound != "No Wound") %>%
  mutate(fully_closed = wound_size_final == 0) %>%
  count(fish, wound, fully_closed) %>%
  pivot_wider(
    names_from = fully_closed,
    values_from = n,
    values_fill = 0,
    names_prefix = "closed_"
  )

closure_xtab %>%
  gt() %>%
  tab_header(
    title = "Fragments reaching full closure (0 cm²) by day 21",
    subtitle = "Counts by treatment"
  ) %>%
  cols_label(
    fish = "Fish",
    wound = "Wound size",
    closed_FALSE = "Still open",
    closed_TRUE = "Fully closed"
  )

cat("\nTotal fully closed: ",
    sum(closure_xtab$closed_TRUE), " / ",
    sum(closure_xtab$closed_TRUE) + sum(closure_xtab$closed_FALSE),
    " wounded fragments\n", sep = "")
```

## Interpretation

The censoring pattern is concentrated in the fish-present treatments
(8/12 closed in Fish/Small and Fish/Large) and rarest in the
no-fish/large treatment (only 3/12). Because the calculated rate uses a
fixed 21-day denominator, it **underestimates** the true healing rate
for the 25 fragments that reached zero before day 21 — and the
underestimation is concentrated in the fish-present cells. The fish
effect we report is therefore **conservative**: with a censoring-aware
estimator the fish effect would be larger, not smaller.

The full day-of-closure time series exists only for the
daily-photographed subset (one fragment per wound × tank). A formal
Cox-style or interval-censored re-analysis on that subset would tighten
the inference and is a target for a future study with daily photography
on all fragments.

------------------------------------------------------------------------

# 6c. Turf-Algae Persistence: Fisher's Exact Test (R1.4)

## Rationale

The manuscript reports parenthetically that turf algae persisted on 7 of
8 algae-colonized corals in no-fish tanks and 1 of 8 in fish tanks by
day 21. Reviewer 1 asked for a formal test. We pull algae status from
the PAM dataset (which records `algae` per coral at day 21) and run a
Fisher's exact test on the 2 × 2 contingency among the 48 **wounded**
corals.

```{r fisher-algae}
# Load algae status from PAM dataset and restrict to wounded corals
pam_for_algae <- read_csv(
  ...
  filter(wound != "No Wound") %>%
  mutate(
    algae_status = ifelse(algae == "yes", "persisted", "cleared")
  )

# 2x2 contingency (fish × algae status)
tab_algae <- with(pam_for_algae, table(fish, algae_status))
print(tab_algae)

# Two-tailed Fisher's exact test
fisher_algae <- fisher.test(tab_algae, alternative = "two.sided")
print(fisher_algae)

# Formatted report
fisher_summary <- tibble(
  Test = "Fisher's exact (fish × algae persistence, wounded corals only)",
  `Odds Ratio` = round(fisher_algae$estimate, 3),
  `95% CI` = paste0("[",
                    round(fisher_algae$conf.int[1], 3), ", ",
                    round(fisher_algae$conf.int[2], 3), "]"),
  `p-value` = format.pval(fisher_algae$p.value, digits = 3)
)

fisher_summary %>%
  gt() %>%
  tab_header(
    title = "Turf algae persistence by fish treatment (day 21)",
    subtitle = "Wounded corals only (n = 48)"
  )
```

The odds ratio \< 1 means fragments with fish were *less* likely to have
persistent algae than fragments without fish. Reciprocal: 1/OR gives the
no-fish-vs-fish odds, which is the more intuitive direction.

## Mechanism discussion (for prose)

The design cannot separate two non-mutually-exclusive mechanisms for
this effect: 1. Fish directly excluding algae via territorial behaviour
or grazing. 2. Faster wound closure in fish-present treatments
shortening the window during which exposed substrate is available for
algal colonization.

Both pathways predict the observed pattern; resolving their relative
contribution would require a design that decouples healing rate from the
presence of the resident fish (e.g., fish present briefly then removed).
